...
ITALY_SUBJECT_FILE = "Italy.csv" 
INDIA_SUBJECT_FILE = "India.csv"
ROOT_IMAGE_DIR = "dataset anemia/"

# Column names inside your CSV files
SUBJECT_ID_COL = "Subject_ID"  # The key linking to the folder names
HB_G_L_COL = "Hgb"        # The raw lab Hb column (in g/dL)
# ----------------------------
...
def build_unified_manifest():
    ...
    if os.path.exists(ITALY_SUBJECT_FILE):
        ...
        # sep=None and engine='python' forces pandas to auto-detect commas vs semicolons
        # encoding='utf-8-sig' automatically destroys hidden Excel BOM characters
        df_italy = pd.read_csv(ITALY_SUBJECT_FILE, sep=None, engine='python', encoding='utf-8-sig')
        df_italy.rename(columns=lambda x: x.strip(), inplace=True)
        
        print("--> Italy columns detected:", df_italy.columns.tolist())
    ...
    if os.path.exists(INDIA_SUBJECT_FILE):
        ...
        df_india = pd.read_csv(INDIA_SUBJECT_FILE, sep=None, engine='python', encoding='utf-8-sig')
        df_india.rename(columns=lambda x: x.strip(), inplace=True)
        
        print("--> India columns detected:", df_india.columns.tolist())
    ...
    # NEW CODE (Keep the values exactly as they are)
    print(f"Loading target column '{HB_G_L_COL}' directly as g/dL...")
    df_master_base['Hb_g_dL'] = df_master_base[HB_G_L_COL]
